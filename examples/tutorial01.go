...
func main() {
	...
	for i := 0; i < int(pFormatContext.NbStreams()); i++ {
		codec := pFormatContext.Streams()[i].CodecParameters().AvCodecGetType()
		fmt.Printf("<><> codec: %v, video type: %v, audio: %v\n", codec, avformat.AVMEDIA_TYPE_VIDEO,
			avformat.AVMEDIA_TYPE_AUDIO)
